...
function App() {
  ...
  const skillCategories = [
    { title: 'OS & Languages', icon: <Code className="text-blue-500 dark:text-blue-400" size={24} />, items: ['Linux (Ubuntu)', 'Bash', 'Python (Learning)', 'Go (Learning)'] },
    { title: 'CI / CD', icon: <GitBranch className="text-pink-500 dark:text-pink-400" size={24} />, items: ['Azure DevOps', 'GitLab CI', 'ArgoCD'] },
    { title: 'Containerization', icon: <Container className="text-teal-500 dark:text-emerald-400" size={24} />, items: ['Docker', 'Kubernetes'] },
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 text-slate-800 dark:bg-[#09090b] dark:text-slate-300 font-sans selection:bg-indigo-200 selection:text-indigo-900 dark:selection:bg-emerald-500/30 dark:selection:text-emerald-200 overflow-hidden transition-colors duration-300">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute left-[-10%] top-[-5%] h-[600px] w-[600px] rounded-full bg-indigo-200/40 dark:bg-emerald-500/20 blur-[120px] dark:blur-[150px] transition-colors duration-500"></div>
        <div className="absolute right-[-5%] bottom-[-5%] h-[500px] w-[500px] rounded-full bg-blue-200/40 dark:bg-cyan-500/10 blur-[120px] dark:blur-[150px] transition-colors duration-500"></div>
      </div>
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <a href="#contact" className="bg-slate-900 hover:bg-slate-800 text-white dark:bg-white dark:hover:bg-slate-200 dark:text-black px-4 py-2 rounded-md text-sm font-bold transition-colors shadow-md dark:shadow-none">
              Hire Me
            </a>
          </div>
        </div>
        ...
        <section id="about" className="py-20 animate-fade-in flex flex-col-reverse md:flex-row items-center justify-between gap-12">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#contact" className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:text-slate-950 px-7 py-3.5 rounded-xl font-bold transition-all shadow-lg shadow-indigo-200 dark:shadow-none">
                <Mail size={18} /> Let's Connect
              </a>
              <a href="https://github.com" target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-900 border border-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 dark:text-white dark:border-slate-800 px-7 py-3.5 rounded-xl font-bold transition-all shadow-sm dark:shadow-none">
          ...
          <div className="w-56 h-56 md:w-80 md:h-80 shrink-0 relative group">
            ...
            <img 
              ...
              className="relative w-full h-full object-cover rounded-[2rem] border-4 border-white dark:border-slate-800 dark:bg-slate-900 shadow-2xl transition-colors duration-300"
            />
          </div>
        </section>
        ...
        <section id="skills" className="py-20 border-t border-slate-100 dark:border-slate-800/50 animate-fade-in transition-colors duration-300" style={{ animationDelay: '100ms', animationFillMode: 'both' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {skillCategories.map((category, idx) => (
              <div key={idx} className="bg-white border border-slate-100 p-8 rounded-2xl hover:shadow-xl hover:shadow-slate-100/50 dark:bg-slate-900/40 dark:border-slate-800/80 dark:hover:bg-slate-900/80 dark:hover:shadow-none transition-all duration-300">
                ...
                <div className="flex flex-wrap gap-2">
                  {category.items.map((item, i) => (
                    <span key={i} className="bg-slate-100 text-slate-700 border border-slate-200/60 dark:bg-slate-950 dark:text-slate-300 dark:border-slate-800 text-sm px-4 py-1.5 rounded-full font-medium transition-colors">
                      {item}
                    </span>
                  ))}
                </div>
        ...
        <section id="projects" className="py-20 border-t border-slate-100 dark:border-slate-800/50 animate-fade-in transition-colors duration-300" style={{ animationDelay: '200ms', animationFillMode: 'both' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {projects.map((project, idx) => (
              <div key={idx} className="bg-white border border-slate-100 rounded-2xl p-8 hover:-translate-y-2 hover:shadow-2xl hover:shadow-indigo-100/50 dark:bg-slate-900/40 dark:border-slate-800/80 dark:hover:border-emerald-500/30 dark:hover:shadow-none transition-all duration-300 group flex flex-col">
                ...
                <div className="flex flex-wrap gap-2 mt-auto font-mono">
                  {project.tags.map((tag, i) => (
                    <span key={i} className="text-xs font-semibold text-indigo-700 bg-indigo-50 border border-indigo-100 dark:text-slate-300 dark:bg-slate-800/80 dark:border-transparent px-2.5 py-1 rounded-md transition-colors">
                      {tag}
                    </span>
                  ))}
                </div>
